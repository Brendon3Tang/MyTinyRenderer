...
public:
    ...
    // mat<DimRows,DimCols,T> inverse() { 
    //     assert(DimRows==DimCols);
    //     // augmenting the square matrix with the identity matrix of the same dimensions a => [ai]
    //     //Matrix result(rows, cols*2);
    //     mat<DimRows,2*DimCols,T> result;
    //     for(int i=0; i<DimRows; i++)
    //         for(int j=0; j<DimCols; j++)
    //             result[i][j] = rows[i][j];
    //     for(int i=0; i<DimRows; i++)
    //         result[i][i+DimCols] = 1;
    //     // first pass
    //     for (int i=0; i<DimRows-1; i++) {
    //         // normalize the first row
    //         for(int j=result.colNum-1; j>=0; j--)
    //             result[i][j] /= result[i][i];
    //         for (int k=i+1; k<DimRows; k++) {
    ...
    //     for(int j=result.colNum-1; j>=DimRows-1; j--)
    //         result[DimRows-1][j] /= result[DimRows-1][DimRows-1];
    //     // second pass
    ...
    //     mat<DimRows,DimCols,T> truncate;
    //     for(int i=0; i<DimRows; i++)
    //         for(int j=0; j<DimCols; j++)
    //             truncate[i][j] = result[i][j+DimCols];
    //     return truncate;
    // }
};
