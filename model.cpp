...
Model::Model(const char *filename) : verts_(), faces_(), norms_(), uv_() {
    ...
    load_texture(filename, "_nm_tangent.tga",      normalmap_);//load tangent normalmap
...
TGAColor Model::diffuse(Vec2f uvf) {
    ...
    return diffusemap_.get(uv[0], uv[1]);//diffusemap_就是有纹理图的TGAImage。得到纹理图中(u,v)点的颜色数据并返回
}
...
Vec2f Model::uv(int iface, int nthvert) {
    return uv_[faces_[iface][nthvert][1]];//faces_[iface][nthvert][1]:取得当前face中当前vertex的纹理坐标的索引值(v/vt/vn中的vt，所以index是1)
}
